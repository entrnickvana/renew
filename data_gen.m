...
% Extract bits as seperate streams from
% Remove even nibbles
...
% reconstruct the temp_data by summing bit shifted streams
...
for idx = 0:num_symbols-1
  curr_fd_sym = fftshift( fd_data_check( idx*(symbol_length) + (1:symbol_length) ) );
  curr_td_sym = ifft(curr_fd_sym) * 2^15;
  % 'read-in' symbol
  curr_td_sym_check = td_data(pad_length + idx*(cp_length + symbol_length) + ((cp_length+1):(symbol_length+cp_length)) );
  td_tx_data = [td_tx_data;  curr_td_sym(end - cp_length + 1:end); curr_td_sym];
  figure()
  subplot(311)
  plot(real(curr_td_sym), "o-"); grid on;
  subplot(312)
...
td_tx_data = [zeros(pad_length,1); td_tx_data; zeros(pad_length,1) ];
